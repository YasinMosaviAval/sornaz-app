...
class _BrowserMusicPlayerState extends State<BrowserMusicPlayer> {
  ...
  Future<void> choose() async {
    ...
    if (mounted && result != null)
      setState(() => files.addAll(result.files.where((f) => f.bytes != null)));
  }
  ...
  Future<void> play(int index) async {
    try {
        ...
        await player.setAudioSource(
          AudioSource.uri(
            Uri.dataFromBytes(
              files[index].bytes!,
              mimeType:
                  'audio/${files[index].extension == 'mp3' ? 'mpeg' : files[index].extension ?? 'mpeg'}',
            ),
          ),
        );
        if (!mounted) return;
      ...
      if (mounted) socialError(context, error);
    }
  }
    ...
    body: Column(
      children: [
        ...
        FilledButton.icon(
          ...
          label: Text(
            socialText(context, 'انتخاب فایل صوتی', 'Choose audio files'),
          ),
        ),
        Expanded(
        ...
        if (selected >= 0)
          StreamBuilder<Duration>(
            ...
            builder: (context, snapshot) => Slider(
              max: (player.duration?.inMilliseconds ?? 1)
                  .clamp(1, 1 << 40)
                  .toDouble(),
